...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // ── Head-to-Head film voting tables (migration 006) ─────────────────────
    ...
    await sql`
      CREATE INDEX IF NOT EXISTS idx_h2h_films_elo_rating
        ON h2h_films (elo_rating DESC)
    `
    ...
    await sql`
      CREATE INDEX IF NOT EXISTS idx_h2h_matchups_film_b_id
        ON h2h_matchups (film_b_id)
    `
    ...
    await sql`
      CREATE INDEX IF NOT EXISTS idx_h2h_matchup_votes_user_id
        ON h2h_matchup_votes (user_id)
    `

    // ── Seed: initial H2H film pool ─────────────────────────────────────────
    ...
    // ── Generate all matchup pairs from seeded films ────────────────────────
    // Uses a self-join to create every unordered pair (a.id < b.id ensures
    // canonical ordering). ON CONFLICT DO NOTHING is idempotent.
